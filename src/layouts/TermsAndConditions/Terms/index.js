...
function Terms() {
  ...
  const getTermsAndConditions = () => {
    ...
    try {
      axiosInstanceAuth
        .get(`/admin/terms_and_conditions/get`)
        .then((res) => {
          if (res.data.status) {
            setLoading(false);
  ...
  const addTermsAndConditions = (e, index) => {
    e.preventDefault();
    try {
      ...
      axiosInstanceAuth
        .post(`/admin/terms_and_conditions/add`, {
          data: encryptedData,
        })
        .then((res) => {
          let pwd = res.data.message;
                ...
                <Card id="account" pt={10}>
                  ...
                  <MDBox p={2}>
                    <MDBox
                      component="form"
                      role="form"
                      onSubmit={(e) => addTermsAndConditions(e, index)}
                    >
                      {/* <Grid container>
                    <Grid item xs={12}>
                      <MDBox
